...
int main() {
  ...
  //  setConsoleOutHook(NewOutputRoutine);
    ...
    printf("Z80DAD'S FLOATING POINT CALC\n ");
    printf("&? FOR HELP\n");
    while (1) {
        ...
        if (IS_SINGLE_OP(op[0]) && (countElements(&rpnStack) >= 1)) {
            ...
            if(op[1] > 97) op[1] = op[1] - 32; // check for capitalized
             switch(op[1]) {
                ...
                case '?':
                    printf("STACK = 8 OPERANDS,\n");
                    printf("OVR.FLOW REST. STACK.\n\n");
                    printf("DOUBLE OPERAND:\n");
                    printf(" + <- ADD\n");
                    printf(" - <- SUBTRACT\n");
                    printf(" * <- MULTIPLY\n");
                    printf(" / <- DIVISON\n");
                    printf("SINGLE OPERAND (ASSUMES RAD):\n");
                    printf(" - !R -> SQUARE \n");
                    printf(" - !S -> SINE \n");
                    printf(" - !C -> COSINE \n");
                    printf(" - !T -> TANGENT \n");
                    printf(" - !L -> COMPUTE LOG10 (SLOW!)\n");
                    printf(" - !N -> COMPUTE LOG (SLOW!)\n");
                    printf("<MORE>"); waitkey(TRUE); printf("\n");
                    printf("NON OPERAND OPERATORS:\n");
                    printf(" - &D -> DISPLAY THE STACK\n");
                    printf(" - &I -> INITIALIZE (AC BUTTON)\n");
                    printf(" - &Q -> QUIT\n");
                    printf(" - &? -> THIS SCREEN\n");
                    break;
